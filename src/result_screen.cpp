...
void result_screen::draw() {
	...
	DrawTextExScaled(aller_r, ("Played by " + results.player_name + " on " + std::format("{:%d.%m.%Y %H:%M:%S}", time) + ".").c_str(), { 4, 64 }, 24, 0, WHITE);
	DrawTextExScaled(aller_r, results.beatmap_header.c_str(), { 4, 4 }, 36, 0, WHITE);
	DrawTextExScaled(aller_r, results.beatmap_header_2.c_str(), { 4, 40 }, 24, 0, WHITE);
	DrawTextExScaled(aller_r, "Ranking", { 960, 4 }, 108, 0, WHITE);
}
